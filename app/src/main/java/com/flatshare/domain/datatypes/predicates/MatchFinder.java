...
class MatchFinder {
    ...
    public boolean apMatchesTenantFilter(ApartmentUserProfile a, TenantFilterSettings tf) {
        return (a.getArea() >= tf.getAreaFrom() && a.getArea() <= tf.getAreaTo())
                ...
                && ((tf.getInternet() == 2) || (!a.hasInternet() && tf.getInternet() == 0) || (a.hasInternet() && tf.getInternet() == 1))
                && ((tf.wantsWashingMachine() == 2) || (!a.hasWaschingMachine() && tf.wantsPurposeApartment() == 0) || (a.hasWaschingMachine() && tf.wantsPurposeApartment() == 1))
